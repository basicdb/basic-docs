...
- `db`: The database object, which you can use to [read, add, update, and delete items](/sdk-reference/react-sdk#db)
- `dbStatus`: Fetches the status of the database
- `user`: Fetches the user object containing the user's ID, email, and name
- `getToken`: Fetches the user's JWT token, returns a promise
